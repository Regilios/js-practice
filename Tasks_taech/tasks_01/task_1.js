 ...
 * в JavaScript типизированные значения, а не типизированные переменные. Доступны следующие встроенные типы:
    ...
    💡 string (строка)
    💡 number (число)
        String — это тип данных для текста (последовательность символов), а Symbol — это уникальный, неизменяемый примитивный тип, созданный для уникальных идентификаторов (ключей объектов). 
        Основные отличия: String — это обычный текст, Symbol — это «секретный» уникальный ключ, который не может быть случайно совпаден или преобразован в строку или число без явного указания. 
    💡 boolean (логическое значение)
    💡 null и undefined (пустое значение)
        В JavaScript undefined означает, что переменной ещё не присвоено значение (не инициализирована), тогда как null — это намеренно присвоенное значение, которое представляет собой явное отсутствие объекта или «пустоту». 
        undefined обычно появляется автоматически (при объявлении без значения или в функциях, не возвращающих явно), а null используется программистом для обозначения отсутствия объекта там, где он ожидается
    💡 object (объект)
