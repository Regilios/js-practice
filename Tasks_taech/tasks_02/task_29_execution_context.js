 ...
 *  JavaScript работает в двух фазах :
    ...
    1.  Глобальный Execution Context создается первым
        ...
        GlobalLexicalEnvironment = {  - глобальная область видимости
            environmentRecord: {
                x: 1,
                foo: [function] - декларация функции
            },
    ...
    Основное различие между переменным окружением (variable environment) и лексическим окружением (lexical environment) в том, что переменное окружение относится конкретно к хранению переменных и функций, 
    объявленных в текущем контексте выполнения, в то время как лексическое окружение включает в себя, помимо переменных и функций, также ссылку на внешнюю лексическую среду, что необходимо для работы замыканий и областей видимости. 

    Переменное окружение: (VariableEnvironment)
        Это структура данных, которая хранит значения переменных и функций, объявленных в текущей области видимости. Например, в функции переменное окружение будет содержать все локальные переменные и функции, объявленные внутри этой функции.
    Лексическое окружение: (LexicalEnvironment)
        Это более широкое понятие, которое включает в себя не только переменные и функции текущей области видимости, но и ссылку на лексическое окружение внешней области видимости (которая была родительской, когда создавалась текущая). 
        Это и есть основа для работы лексической области видимости (или замыканий). Благодаря этой ссылке, функция может "помнить" переменные из своей лексической родительской области, даже если она была вызвана за пределами этой области.
    В итоге: Переменное окружение — это часть лексического окружения, которая отвечает за хранение переменных и функций текущей области видимости, а лексическое окружение включает в себя эту часть, 
        а также ссылку на внешнюю область видимости, что критически важно для работы замыканий. 
 */